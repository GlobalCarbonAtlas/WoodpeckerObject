...
var Woodpecker = Class.create( {
    ...
    {
        // Parameters
        this.graphContainerId = parameters.graphContainerId;
        this.legendContainerId = parameters.legendContainerId;
        this.graphContainer = $( "#" + this.graphContainerId );
        this.legendContainer = $( "#" + this.legendContainerId );
        ...
        // Dimensions
        this.graphMargin = {"top": 10, "right": 5, "bottom": 10, "left": 10};
        this.axisTextDimension = {"yAxisWidth" : 60, "xAxisHeight": 40};
        this.graphSvgWidth = this.graphContainer.width() ? this.graphContainer.width() : this.initContainer( this.graphContainer, true, 500 );
        this.graphSvgHeight = this.graphContainer.height() ? this.graphContainer.height() : this.initContainer( this.graphContainer, false, 400 );
        if( this.displayIconsMenu )
            this.graphSvgHeight = this.graphSvgHeight - 20;
        this.legendSvgWidth = this.legendContainer.width() ? this.legendContainer.width() : this.initContainer( this.legendContainer, true, 500 );
        this.legendSvgHeight = this.legendContainer.height() ? this.legendContainer.height() : this.initContainer( this.legendContainer, false, 100 );
        this.plotSize = {
            "width":  this.graphSvgWidth - this.graphMargin.left - this.graphMargin.right - this.axisTextDimension.yAxisWidth,
            "height": this.graphSvgHeight - this.graphMargin.top - this.graphMargin.bottom - this.axisTextDimension.xAxisHeight - 10
        };
    ...
    initContainer: function( container, isWidth, value )
    {
        if( isWidth )
...
// **************************************************************
    createGraph: function()
    {
        ...
        this.graphContainer.append( divToCloneGraph );
        if( this.displayIconsMenu )
...
// **************************************************************
    ...
    createOrUpdateAxis: function()
    {
        ...
        this.xAxis.ticks( this.graphSvgWidth / 100 ).tickSize( -(this.plotSize.height), 0 );
        this.yAxis.ticks( this.graphSvgHeight / 36 ).tickSize( -(this.plotSize.width), 0 );

        var g = d3.select( 'g.wrap' );

        // X axis
        //this.xAxisLabelText = "Flux/kgC/m2/h/m33/g/m2/m2/d";
        this.xPosition = this.x.range()[1] / 2 - getTextWidth( this.graphContainerId, this.xAxisLabelText ) / 2;
        ...
        this.xPosition = -this.y.range()[0] / 2 - getTextWidth( this.graphContainerId, this.yAxisLabelText ) / 2;
...
// **************************************************************
    ...
    bindZoomsToGraph: function()
    {
        ...
        this.graphContainer.on( wheelEventAllBrowsers, jQuery.proxy( function()
        {
            this.bindZoom();
        }, this ) );

        this.zoomRects.select( 'rect.zoom.x.box' ).call( d3.behavior.zoom().x( this.x ).on( "zoom", jQuery.proxy( this.redrawAfterPanOrZoom, this ) ) );
        this.zoomRects.select( 'rect.zoom.y.box' ).call( d3.behavior.zoom().y( this.y ).on( "zoom", jQuery.proxy( this.redrawAfterPanOrZoom, this ) ) );
    },
...
// **************************************************************
    ...
    createOrUpdateContextMenu: function()
    {
        ...
        this.graphContainer.contextmenu( option, "WPrightMenu" + this.graphContainerId, menuTitleDiv, true );
    },
    ...
    createOrUpdateIconsMenu: function()
    {
        ...
        else
        {
            ...
            this.graphContainer.append( divMenu );
        }
